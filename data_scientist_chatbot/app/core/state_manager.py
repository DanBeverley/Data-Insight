...
def create_workflow_status_context(workflow_context: Dict[str, Any], current_event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if (
            hasattr(builtins, "_session_store")
            and session_id in builtins._session_store
            and "dataframe" in builtins._session_store[session_id]
        ):
...
def create_lean_hands_context(state, task_description: str, session_id: str) -> Dict[str, Any]:
    ...
    for i in range(len(messages) - 1, max(-1, len(messages) - 3), -1):
        ...
        if hasattr(msg, "content") and hasattr(msg, "type") and msg.type == "ai":
            ...
            break
    task_message = (
        f"{brain_context}\n\nExecute this technical task: {task_description}"
        if brain_context
        else f"Execute this technical task: {task_description}"
    )
    ...
    logger.debug(
        f"Context pruning - Original: {original_msg_count} messages, Pruned: {lean_msg_count} messages ({reduction:.1f}% reduction)"
    )
