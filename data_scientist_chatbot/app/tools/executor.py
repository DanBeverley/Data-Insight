...
def format_training_result(result: dict) -> str:
    ...
    env_label = {"cpu": "CPU (E2B Sandbox)", "gpu_azure": "GPU (Azure ML)", "gpu_aws": "GPU (AWS SageMaker)"}.get(
        env, env
    )
...
def execute_tool(tool_name: str, tool_args: dict, session_id: str) -> str:
    ...
    if tool_name == "python_code_interpreter":
        ...
        try:
            from data_scientist_chatbot.app.core.training_executor import (
                training_executor,
                should_use_training_executor,
            )
...
def knowledge_graph_query_logic(query: str, session_id: str) -> str:
    try:
        ...
        performance_monitor.record_metric(
            session_id=session_id, metric_name="graph_query", value=1.0, context={"query": query}
        )
        if graph_data:
...
def access_learning_data_logic(query: str, session_id: str) -> str:
    try:
        ...
        performance_monitor.record_metric(
            session_id=session_id, metric_name="learning_data_access", value=1.0, context={"query": query}
        )
...
def web_search_logic(query: str, session_id: str) -> str:
    try:
        ...
        performance_monitor.record_metric(
            session_id=session_id, metric_name="web_search_used", value=1.0, context={"query": query}
        )
        return search_web(query)
    except Exception as e:
        performance_monitor.record_metric(
            session_id=session_id, metric_name="web_search_error", value=1.0, context={"error": str(e), "query": query}
        )
...
def retrieve_historical_patterns_logic(task_description: str, session_id: str) -> str:
    try:
        patterns = context_manager.get_cross_session_patterns(pattern_type=task_description, limit=3)
        performance_monitor.record_metric(
            session_id=session_id,
...
def zip_artifacts_logic(artifact_ids: list, description: str, session_id: str) -> str:
    try:
        ...
        performance_monitor.record_metric(
            session_id=session_id, metric_name="zip_artifacts_error", value=1.0, context={"error": str(e)}
        )
...
def load_trained_model_logic(model_type: str, model_id: str, session_id: str) -> str:
    ...
    try:
        ...
        performance_monitor.record_metric(
            session_id=session_id, metric_name="model_load_error", value=1.0, context={"error": str(e)}
        )
