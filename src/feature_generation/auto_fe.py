...
class AutomatedFeatureEngineer:
    ...
    def generate_features(
        ...
        logging.info(
            f"Feature generation complete. Memory usage: {end_memory:.1f}MB (+{end_memory-start_memory:.1f}MB)"
        )
    ...
    def _add_ecommerce_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        amount_cols = [
            col
            for col in df.columns
            if any(pattern in col.lower() for pattern in ["price", "amount", "cost", "revenue"])
        ]

        # Find quantity columns
        qty_cols = [
            col for col in df.columns if any(pattern in col.lower() for pattern in ["quantity", "qty", "count"])
        ]
    ...
    def _add_finance_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        amount_cols = [
            col for col in df.columns if any(pattern in col.lower() for pattern in ["amount", "balance", "transaction"])
        ]
        ...
        for col in text_cols[:3]:  # Limit to prevent explosion
            ...
            if TEXTBLOB_AVAILABLE and len(text_series) <= 1000:  # Limit for performance
                try:
                    ...
                    subjectivity = text_series.apply(
                        lambda x: TextBlob(str(x)[:500]).sentiment.subjectivity if x else 0
                    )
            ...
            if SKLEARN_TEXT_AVAILABLE and len(text_series.unique()) > 10:
                try:
                    ...
                    vectorizer = TfidfVectorizer(
                        max_features=50, stop_words="english", min_df=2, max_df=0.8, ngram_range=(1, 2)
                    )
        ...
        for col in datetime_cols:
            ...
            if len(numeric_cols) > 0 and len(df) > 30:  # Sufficient data for patterns
                ...
                for num_col in numeric_cols[:3]:  # Limit to prevent explosion
                    ...
                    for lag in [1, 7, 30]:
                        if lag < len(df):
                            df[f"temporal_{col}_{num_col}_lag_{lag}"] = (
                                df_sorted[num_col].shift(lag).iloc[sorted_idx.argsort()]
                            )
                    ...
                    for window in [7, 30]:
                        if window < len(df):
                            ...
                            df[f"temporal_{col}_{num_col}_rolling_mean_{window}"] = rolling_mean.iloc[
                                sorted_idx.argsort()
                            ]
                            df[f"temporal_{col}_{num_col}_rolling_std_{window}"] = rolling_std.iloc[
                                sorted_idx.argsort()
                            ]
    ...
    def _generate_relational_features(
        self, dataframes: Dict[str, pd.DataFrame], relationships: List[Tuple[str, str, str, str]], target_entity: str
    ) -> pd.DataFrame:
        ...
        for entity_name, df_original in dataframes.items():
            ...
            self.entityset.add_dataframe(dataframe_name=entity_name, dataframe=df, index=index_col, make_index=False)
...
class InteractionFeatureGenerator(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y=None):
        ...
        if y is not None and len(numeric_cols) > 1:
            ...
            self.interaction_pairs_ = [(col1, col2) for _, col1, col2 in correlations[: self.max_interactions]]
        else:
            import itertools
...
def create_feature_engineering_config(
    df: pd.DataFrame, task_type: str = "regression", complexity: str = "medium"
) -> Dict[str, Any]:
    ...
    logging.info(
        f"Feature engineering config: max_features={config['max_features']}, "
