...
class ComplianceManager:
    ...
    def update_consent(self, subject_id: str, purpose: ProcessingPurpose, 
                      consent_given: bool) -> bool:
    ...
    def _determine_legal_basis(self, purpose: ProcessingPurpose, 
    ...
    def _revoke_processing_consent(self, subject_id: str, purpose: ProcessingPurpose):
        for record in self.processing_records.values():
            if (subject_id in record.data_subject_ids and 
                record.processing_purpose == purpose and 
                record.processing_end is None):
    ...
    def _check_deletion_eligibility(self, subject_id: str) -> Dict[str, Any]:
        ...
        legal_holds = [
            ...
            if record.legal_basis in ["legal_obligation", "vital_interests"]
        ]
        ...
        return {
            "eligible": True,
            "affected_records": [r.record_id for r in active_records]
        }
    ...
    def export_subject_data(self, subject_id: str) -> Dict[str, Any]:
        ...
        related_records = [
            record for record in self.processing_records.values()
            if subject_id in record.data_subject_ids
        ]
    ...
    def _check_retention_violations(self) -> List[ComplianceViolation]:
        ...
        for subject_id, subject in self.data_subjects.items():
            if (subject.data_retention_date and 
                datetime.now() > subject.data_retention_date and 
                not subject.deletion_requested):
    ...
    def _calculate_consent_statistics(self) -> Dict[str, Any]:
        ...
        for purpose in ProcessingPurpose:
            consented = sum(1 for subject in self.data_subjects.values() 
                          if subject.consent_status.get(purpose, False))
    ...
    def process_regulatory_request(self, request_type: str, subject_id: str, 
