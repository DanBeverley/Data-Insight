...
class PrivacyEngine:
    ...
    def _assess_generalization_level(self, df: pd.DataFrame) -> float:
        ...
        for column in df.columns:
            if df[column].dtype == "object":
                if any(
                    indicator in str(df[column].iloc[0]).lower() for indicator in ["*", "other", "unknown", "range"]
                ):
    ...
    def _calculate_column_sensitivity(
        self, df: pd.DataFrame, quasi_identifiers: List[str], sensitive_attributes: List[str]
    ) -> Dict[str, float]:
        column_sensitivity = {}
        ...
        for column in affected_columns:
            ...
            if original_df[column].dtype in ["int64", "float64"]:
                ...
                try:
                    ...
                    distribution_similarity = sum(
                        min(orig_counts.get(val, 0), priv_counts.get(val, 0)) for val in common_values
                    )
    ...
    def generate_synthetic_data(
        self, df: pd.DataFrame, num_samples: Optional[int] = None
    ) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        for column in df.columns:
            if df[column].dtype in ["int64", "float64"]:
                ...
                synthetic_values = np.random.choice(value_counts.index, size=num_samples, p=value_counts.values)
    ...
    def apply_comprehensive_privacy_protection(
        ...
        if sensitive_attributes and len(sensitive_attributes) > 0:
            for sensitive_attr in sensitive_attributes[:1]:
                protected_df, ld_metadata = self.apply_l_diversity(protected_df, quasi_identifiers, sensitive_attr)
