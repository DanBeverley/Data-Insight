...
class IntelligentDataProfiler:
    ...
    def _infer_datetime_format(self, series: pd.Series) -> Optional[str]:
        ...
        for fmt in common_formats:
            try:
                parsed_count = sum(
                    1 for val in sample_values if pd.to_datetime(val, format=fmt, errors="coerce") is not pd.NaT
                )
            ...
            for col2, profile2 in key_columns.items():
                if col1 != col2:
                    ...
                    if overlap_ratio > 0.1:  # Significant overlap
                        profile1.evidence["potential_relationships"] = profile1.evidence.get(
                        ...
                        profile1.evidence["potential_relationships"].append(
                            {
                                ...
                                "relationship_type": (
                                    "potential_fk"
                                    if profile1.semantic_type == SemanticType.FOREIGN_KEY
                                    else "related_key"
                                ),
                            }
                        )
    ...
    def _generate_overall_recommendations(
        self, column_profiles: Dict, domain_matches: List, relationships: List
    ) -> List[str]:
        ...
        high_null_cols = [
            col for col, profile in column_profiles.items() if profile.evidence.get("null_ratio", 0) > 0.3
        ]
        if high_null_cols:
        ...
        if domain_matches:
            ...
            recommendations.append(
                f"Dataset appears to be {top_domain}-related - apply domain-specific feature engineering"
            )
