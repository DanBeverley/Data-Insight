...
class HybridDataProfiler:
    ...
    def generate_comprehensive_profile(self, df: pd.DataFrame, 
                                     ...
                                     context: Optional[Dict[str, Any]] = None) -> DataProfileSummary:
        """
        ...
        logging.info(f"Starting comprehensive profiling of dataset with {len(df)} samples and {len(df.columns)} features")
        ...
        try:
            ...
            profile_summary = DataProfileSummary(
                ...
                quality_assessment={
                    ...
                    'dimension_scores': {dim.value: asdict(score) for dim, score in quality_report.dimension_scores.items()} if quality_report else {},
    ...
    def _generate_dataset_insights(self, df: pd.DataFrame, 
                                  semantic_profile: Dict[str, Any],
                                  quality_report,
                                  anomaly_summary: Dict[str, Any],
                                  context: Dict[str, Any]) -> DatasetInsights:
        ...
        for col_name, profile in column_profiles.items():
            semantic_type = profile.semantic_type.value if hasattr(profile.semantic_type, 'value') else str(profile.semantic_type)
    ...
    def _create_agent_context(self, df: pd.DataFrame,
                             semantic_profile: Dict[str, Any],
                             quality_report,
                             anomaly_results,
                             missing_analysis,
        ...
        for col_name in df.columns:
            ...
            if col_name in semantic_profile['column_profiles']:
                ...
                basic_info['semantic_type'] = profile.semantic_type.value if hasattr(profile.semantic_type, 'value') else str(profile.semantic_type)
        ...
        agent_context = {
            ...
            'data_quality': {
                ...
                'missing_data_summary': {
                    ...
                    'missing_percentages': {col: round((df[col].isnull().sum() / len(df)) * 100, 2) 
                                          for col in df.columns if df[col].isnull().any()}
                }
            },
            'anomalies': {
                'total_count': len(anomaly_results[0].anomaly_indices) if anomaly_results and len(anomaly_results) > 0 else 0,
                'affected_columns': list(set([col for result in anomaly_results for col in result.affected_features])) if anomaly_results else []
            },
            ...
            'code_generation_hints': {
                ...
                'suggested_target_columns': [col for col in df.columns if pd.api.types.is_numeric_dtype(df[col]) and df[col].nunique() > 2][:3]
            }
        }
    ...
    def _generate_visualization_suggestions(self, df: pd.DataFrame, 
                                          semantic_profile: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _generate_analysis_recommendations(self, df: pd.DataFrame,
                                         semantic_profile: Dict[str, Any],
    ...
    def _generate_comprehensive_recommendations(self, semantic_profile: Dict[str, Any],
                                              quality_report,
                                              anomaly_results,
                                              missing_analysis,
...
def generate_dataset_profile_for_agent(df: pd.DataFrame, 
                                      reference_df: Optional[pd.DataFrame] = None,
                                      context: Optional[Dict[str, Any]] = None,
                                      config: Optional[Dict[str, Any]] = None) -> DataProfileSummary:
    """
