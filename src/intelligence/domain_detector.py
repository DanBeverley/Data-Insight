...
class DomainDetector:
    ...
    def detect_domain(self, df: pd.DataFrame, column_profiles: Optional[Dict] = None) -> List[DomainMatch]:
        ...
        for domain, signatures in self.domain_signatures.items():
            ...
            if confidence > 0.3:
                ...
                matches.append(
                    DomainMatch(
                        domain=domain, confidence=confidence, matched_columns=matched_columns, evidence=evidence
                    )
                )
    ...
    def _calculate_domain_confidence(
        self, df: pd.DataFrame, domain: str, signatures: Dict[str, List[str]], column_profiles: Optional[Dict] = None
        ...
        for col in df.columns:
            if df[col].dtype == "object":
                ...
                for domain_value in domain_values:
                    pattern_matches = sum(
                        1 for val in unique_values if domain_value.lower() in val or val in domain_value.lower()
                    )
    ...
    def get_domain_recommendations(self, domain_matches: List[DomainMatch]) -> Dict[str, List[str]]:
        ...
        if not domain_matches:
            return {
                "general": ["Consider adding domain-specific identifiers", "Add temporal columns for trend analysis"]
            }
