...
class MultiLayerAnomalyDetector:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "severity_thresholds": {"low": 0.7, "medium": 0.8, "high": 0.9, "critical": 0.95},
        }
    ...
    def detect_anomalies(self, df: pd.DataFrame, reference_df: Optional[pd.DataFrame] = None) -> List[AnomalyResult]:
        ...
        logging.info(
            f"Detected {len(filtered_results)} anomalies across {len(self.detection_results)} initial detections"
        )
    ...
    def _filter_and_merge_results(self) -> List[AnomalyResult]:
        ...
        filtered = [
            result for result in self.detection_results if result.confidence >= self.config["confidence_threshold"]
        ]
        ...
        for result in filtered:
            ...
            for existing in merged_results:
                ...
                overlap_ratio = len(result_indices_set & existing_indices_set) / len(
                    result_indices_set | existing_indices_set
                )
...
def detect_dataset_anomalies(
    df: pd.DataFrame, reference_df: Optional[pd.DataFrame] = None, config: Optional[Dict[str, Any]] = None
