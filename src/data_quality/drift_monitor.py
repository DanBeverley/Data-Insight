...
class ComprehensiveDriftMonitor:
    ...
    def detect_drift(self, current_df: pd.DataFrame, feature_importance: Optional[Dict[str, float]] = None) -> List[DriftResult]:
        """
    ...
    def _detect_numeric_drift(self, current_series: pd.Series, column: str, timestamp: datetime) -> List[DriftResult]:
        ...
        try:
            ...
            # PSI calculation
            psi = np.sum((current_proportions - ref_stats['reference_proportions']) * 
        ...
        try:
            ...
            ref_sample = np.random.normal(
                ref_stats['mean'], 
                ref_stats['std'], 
                min(len(current_clean), 1000)  # Limit size for efficiency
            )
    ...
    def _detect_categorical_drift(self, current_series: pd.Series, column: str, timestamp: datetime) -> List[DriftResult]:
    ...
    def _detect_feature_importance_drift(self, current_importance: Dict[str, float], 
                                       timestamp: datetime) -> Optional[DriftResult]:
        ...
        try:
            ...
            ref_ranks = {feat: i for i, feat in enumerate(
                sorted(common_features, key=lambda x: ref_importance[x], reverse=True)
            )}
            curr_ranks = {feat: i for i, feat in enumerate(
                sorted(common_features, key=lambda x: current_importance[x], reverse=True)
            )}
...
def monitor_data_drift(reference_df: pd.DataFrame, current_df: pd.DataFrame,
                      ...
                      config: Optional[Dict[str, Any]] = None) -> Tuple[List[DriftResult], Dict[str, Any]]:
    """
