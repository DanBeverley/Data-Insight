...
class ContextAwareQualityAssessor:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "completeness_thresholds": {"excellent": 0.95, "good": 0.85, "acceptable": 0.70, "poor": 0.50},
    ...
    def assess_quality(
        ...
        if len(df) < self.config["min_samples_for_assessment"]:
            raise ValueError(
                f"Insufficient samples for assessment. Need at least {self.config['min_samples_for_assessment']}"
            )
        ...
        report = DataQualityReport(
            ...
            metadata={
                ...
                "drift_summary": {
                    ...
                    "drift_results": [d.description for d in drift_results],
                },
    ...
    def _assess_completeness(self, df: pd.DataFrame, context: Dict[str, Any]) -> QualityScore:
        ...
        if critical_columns:
            recommendations.append(
                f"Prioritize data collection for critical columns: {', '.join(critical_columns[:3])}"
            )
    ...
    def _assess_accuracy(
        ...
        if outlier_details:
            worst_columns = sorted(outlier_details.items(), key=lambda x: x[1]["percentage"], reverse=True)[:3]
            column_details = ", ".join([f"{col}({info['percentage']:.1f}%)" for col, info in worst_columns])
            issues.append(f"Highest outlier columns: {column_details}")
    ...
    def _assess_consistency(self, df: pd.DataFrame, context: Dict[str, Any]) -> QualityScore:
            ...
            try:
                ...
                for i, col1 in enumerate(numeric_cols):
                    for col2 in numeric_cols[i + 1 :]:
                        ...
                        if (
                            self._are_columns_related(col1, col2)
                            and corr_value < self.config["consistency_correlation_threshold"]
                        ):
        ...
        for column in categorical_cols:
            ...
            if similar_values:
                categorical_issues.extend(
                    [f"Inconsistent formatting in {column}: {', '.join(group)}" for group in similar_values[:3]]
                )  # Limit to first 3
