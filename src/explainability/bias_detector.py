...
class BiasDetector:
    ...
    def detect_bias(self, model, X: pd.DataFrame, y_true: np.ndarray, 
                   y_pred: np.ndarray, y_pred_proba: Optional[np.ndarray] = None) -> List[BiasResult]:
        ...
        for attribute in self.sensitive_attributes:
            ...
            group_results = self._analyze_attribute_bias(
                model, X, y_true, y_pred, y_pred_proba, attribute
            )
            bias_results.extend(group_results)
        
        return bias_results
    
    def _analyze_attribute_bias(self, model, X: pd.DataFrame, y_true: np.ndarray, 
                               y_pred: np.ndarray, y_pred_proba: Optional[np.ndarray], 
                               attribute: str) -> List[BiasResult]:
    ...
    def _calculate_group_statistics(self, X: pd.DataFrame, y_true: np.ndarray, 
                                   y_pred: np.ndarray, y_pred_proba: Optional[np.ndarray], 
    ...
    def _check_demographic_parity(self, group_stats: Dict[str, Dict[str, float]], 
                                 attribute: str) -> Optional[BiasResult]:
    ...
    def _check_equalized_opportunity(self, group_stats: Dict[str, Dict[str, float]], 
                                    attribute: str) -> Optional[BiasResult]:
    ...
    def _check_equalized_odds(self, group_stats: Dict[str, Dict[str, float]], 
                             attribute: str) -> Optional[BiasResult]:
        ...
        for group, stats in group_stats.items():
            if (abs(stats['true_positive_rate'] - mean_tpr) > self.fairness_threshold or
                abs(stats['false_positive_rate'] - mean_fpr) > self.fairness_threshold):
    ...
    def _check_calibration(self, group_stats: Dict[str, Dict[str, float]], 
                          attribute: str) -> Optional[BiasResult]:
    ...
    def calculate_fairness_metrics(self, model, X: pd.DataFrame, y_true: np.ndarray, 
                                  y_pred: np.ndarray, y_pred_proba: Optional[np.ndarray] = None) -> FairnessMetrics:
    ...
    def generate_bias_report(self, model, X: pd.DataFrame, y_true: np.ndarray, 
    ...
    def _generate_overall_recommendations(self, bias_results: List[BiasResult], 
                                        fairness_metrics: FairnessMetrics) -> List[str]:
        recommendations = []
