...
class ExplanationEngine:
        ...
        if LIME_AVAILABLE:
            try:
                ...
                self.lime_explainer = lime_tabular.LimeTabularExplainer(
                    self.X_train.values, feature_names=self.feature_names, mode=mode, random_state=42
                )
            except Exception:
                self.lime_explainer = None
    ...
    def _shap_global_explanation(self) -> GlobalExplanation:
        try:
            ...
            return GlobalExplanation(
                feature_importance=feature_importance, feature_interactions=interactions, explanation_method="shap"
            )
        ...
        try:
            if hasattr(self.model, "feature_importances_"):
                ...
                perm_importance = permutation_importance(
                    self.model,
                    self.X_train.sample(min(200, len(self.X_train))),
            ...
            return GlobalExplanation(feature_importance=feature_importance, explanation_method="fallback")

        except Exception:
            uniform_importance = 1.0 / len(self.feature_names)
            feature_importance = {name: uniform_importance for name in self.feature_names}

            return GlobalExplanation(feature_importance=feature_importance, explanation_method="uniform")
    ...
    def generate_business_insights(
        self, global_explanation: GlobalExplanation, sample_instances: Optional[pd.DataFrame] = None
    ) -> List[BusinessInsight]:
        insights = []

        top_features = sorted(global_explanation.feature_importance.items(), key=lambda x: x[1], reverse=True)[:5]
    ...
    def explain_prediction_difference(self, instance1: pd.Series, instance2: pd.Series) -> Dict[str, Any]:
        ...
        for feature in self.feature_names:
            diff = explanation1.feature_contributions.get(feature, 0) - explanation2.feature_contributions.get(
                feature, 0
            )
