...
from src.explainability.bias_detector import BiasDetector, BiasType, SeverityLevel, BiasResult, FairnessMetrics
...
class TestBiasDetector:
    ...
    def test_detect_bias_returns_list(self, balanced_data, balanced_predictions):
        ...
        results = detector.detect_bias(
            model, balanced_data, balanced_predictions["y_true"], balanced_predictions["y_pred"]
        )
    ...
    def test_single_group_skipped(self):
        single_group_data = pd.DataFrame({"feature1": [1, 2, 3], "sensitive_attr": ["Group_A", "Group_A", "Group_A"]})
        detector = BiasDetector(sensitive_attributes=["sensitive_attr"])
        model = Mock()
