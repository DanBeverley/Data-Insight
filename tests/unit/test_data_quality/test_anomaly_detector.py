...
from src.data_quality.anomaly_detector import (
...
class TestMultiLayerAnomalyDetector:
    ...
    def test_iqr_method_detects_outliers(self, detector):
        data = pd.DataFrame({
            'feature1': list(range(1, 20)) + [100]
        })
        detector._compute_feature_statistics(data)
        detector._detect_statistical_outliers(data)

        iqr_results = [r for r in detector.detection_results
                      if r.metadata.get('method') == 'iqr']
    ...
    def test_single_column_dataframe(self, detector):
        df = pd.DataFrame({
            'single_feature': np.concatenate([np.random.normal(100, 10, 50), [500]])
        })
