...
class TestToolChaining:
    ...
    def test_knowledge_graph_then_code_execution(self):
        ...
        with patch('data_scientist_chatbot.app.tools.executor.execute_tool') as mock_exec:
            ...
            assert any("graph" in str(msg.content).lower() or "query" in str(msg.content).lower() for msg in result1["messages"] if isinstance(msg, ToolMessage))
            ...
            state = {
                "messages": [
                    ...
                    AIMessage(
                        ...
                        tool_calls=[{
                            "name": "web_search",
                            "args": {"query": "housing market trends 2024"},
                            "id": "call_web"
                        }]
                    )
                ],
    ...
    def test_multiple_python_executions_in_sequence(self):
        ...
        with patch('data_scientist_chatbot.app.tools.executor.execute_tool') as mock_exec:
            mock_exec.side_effect = [
                ...
                "Visualization created: correlation_heatmap.html"
            ]
    ...
    def test_learning_data_retrieval_before_execution(self):
        ...
        with patch('data_scientist_chatbot.app.tools.executor.execute_tool') as mock_exec:
            mock_exec.return_value = "Learning data: Previous successful executions show using seaborn for visualization"
            ...
            tool_messages = [msg for msg in result["messages"] if isinstance(msg, ToolMessage)]
            assert any("learning" in msg.content.lower() or "data" in msg.content.lower() or len(msg.content) > 0 for msg in tool_messages)
