...
def azure_credentials_available():
    ...
    return all(
        [os.getenv("AZURE_SUBSCRIPTION_ID"), os.getenv("AZURE_RESOURCE_GROUP"), os.getenv("AZURE_WORKSPACE_NAME")]
    )


def aws_credentials_available():
    """Check if AWS credentials are configured"""
    return all([os.getenv("AWS_ACCESS_KEY_ID"), os.getenv("AWS_SECRET_ACCESS_KEY"), os.getenv("AWS_REGION")])
...
class TestAWSGPUIntegration:
    ...
    def test_aws_sagemaker_instance_types(self):
        ...
        response = pricing.get_products(
            ...
            Filters=[{"Type": "TERM_MATCH", "Field": "productFamily", "Value": "ML Instance"}],
            MaxResults=1,
        )
...
"""
        ...
        result = train_on_azure_gpu(
            simple_training_code, "integration_test_session", {"instance_type": "Standard_D2_v2", "timeout": 180}
        )
...
"""
        ...
        result = train_on_aws_gpu(
            simple_training_code, "integration_test_session", {"instance_type": "ml.m5.large", "timeout": 180}
        )
